(function () {
  ...
  function generateId(index) {
    ...
    return (
      generateId(Math.floor(index / 26) - 1) +
  ...
  function calculate() {
    ...
    for (let i = 0; i < activities.length; i++) {
      ...
      const dur =
        typeof a.duration === "number" ? a.duration : parseFloat(a.duration);
      if (isNaN(dur) || dur <= 0) {
  ...
  function renderResults() {
    ...
    if (nonCritical.length > 0) {
      html += `<strong>Non-critical activities:</strong> ${nonCritical
        .map((n) => `${n.id} (float: ${Math.round(n.tf * 10) / 10})`)
        .join(", ")}`;
    }
  ...
  function getCriticalPathSequence(nodes) {
    ...
    const starts = critical.filter(
      (n) =>
        ...
        !n.dependencies.some((d) => critical.find((c) => c.id === d)),
    );
    const path = [];

    function walk(node) {
      path.push(node.id);
      const successors = critical.filter((n) =>
  ...
  function buildDiagramSVG(nodes) {
    ...
    function getLayer(node) {
      ...
      const maxDepLayer = Math.max(
        ...node.dependencies.map((d) => getLayer(nodeMap[d])),
      );
    ...
    const maxNodesInLayer = Math.max(
      ...Object.values(layerGroups).map((g) => g.length),
    );
    ...
    nodes.forEach((node) => {
      node.dependencies.forEach((depId) => {
        ...
        path.setAttribute(
          "marker-end",
          isCritEdge ? "url(#ah-crit)" : "url(#ah-norm)",
        );
    ...
    nodes.forEach((node) => {
      ...
      header.setAttribute(
        "d",
        `M${hx + 6},${hy} h${nodeWidth - 12} a6,6 0 0 1 6,6 v20 h-${nodeWidth} v-20 a6,6 0 0 1 6,-6 z`,
      );
      ...
      sep.setAttribute("x1", hx + 1);
      sep.setAttribute("y1", hy + 26);
      sep.setAttribute("x2", hx + nodeWidth - 1);
      sep.setAttribute("y2", hy + 26);
      ...
      headerText.setAttribute("x", hx + nodeWidth / 2);
      headerText.setAttribute("y", hy + 17);
      headerText.setAttribute("text-anchor", "middle");
      headerText.setAttribute("font-size", "11");
      headerText.setAttribute("font-weight", "700");
      headerText.setAttribute("fill", textColor);
      ...
      nameText.setAttribute("x", hx + nodeWidth / 2);
      nameText.setAttribute("y", hy + 17);
      ...
      [
        [hx + 1, hy + 26, midX - 1, hy + 26 + 18],
        [hx + 1, hy + 26 + 18, hx + nodeWidth - 1, hy + 26 + 18],
        [midX, hy + 26, midX, hy + 26 + 36],
        [hx + 1, hy + 26 + 36, hx + nodeWidth - 1, hy + 26 + 36],
      ...
      vLine.setAttribute("y1", hy + 26);
      vLine.setAttribute("x2", midX);
      vLine.setAttribute("y2", hy + 62);
      ...
      [
        ["ES", round(node.es), leftX, dataY1],
        ["EF", round(node.ef), rightX, dataY1],
        ["LS", round(node.ls), leftX, dataY2],
        ["LF", round(node.lf), rightX, dataY2],
      ].forEach(([label, val, x, y]) => {
        const lbl = document.createElementNS(ns, "text");
        lbl.setAttribute("x", x);
        lbl.setAttribute("y", y);
        lbl.setAttribute("font-size", "8");
        lbl.setAttribute("fill", "#666666");
        lbl.setAttribute("font-family", "-apple-system, sans-serif");
        lbl.textContent = label;
        g.appendChild(lbl);

        const valText = document.createElementNS(ns, "text");
        valText.setAttribute("x", x + 18);
      ...
      durBg.setAttribute("x", hx + 1);
      durBg.setAttribute("y", hy + 62);
      durBg.setAttribute("width", nodeWidth - 2);
      durBg.setAttribute("height", nodeHeight - 62 - 1);
      durBg.setAttribute("fill", "#f8f8f8");
      g.appendChild(durBg);

      // Duration text
      const durText = document.createElementNS(ns, "text");
      durText.setAttribute("x", hx + nodeWidth / 2);
      durText.setAttribute("y", hy + 82);
      durText.setAttribute("text-anchor", "middle");
      durText.setAttribute("font-size", "10");
      durText.setAttribute("font-weight", "600");
      durText.setAttribute("fill", "#161616");
      ...
      border.setAttribute("height", nodeHeight);
  ...
  // --- Initialise with sample data (PERT off, durations entered directly) ---
  activities = [
    {
      name: "Requirements",
      optimistic: "",
  ...
  // Sample data uses direct durations, so start with PERT estimation off
  pertToggle.checked = false;
